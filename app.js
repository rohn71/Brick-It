const cvs = document.getElementById("brickit");
...
BG_SOUND.src ="./sounds/BG.mp3"
OVER_SOUND.src ="./sounds/gameover.mp3";
...
function showGamePoints1(text, textX, textY){
    ctx.fillStyle = "red";
    ctx.font = "25px Germania One";
    ctx.fillText(text, textX, textY);
...
function gameOver(){
    if(LIFE <= 0){
        ...
        BG_SOUND.pause();
        OVER_SOUND.play();
        showGamePoints1("Game Over", cvs.width/2 - 40, cvs.height/2  -20); 
        showGamePoints1("Refresh to Play Again!", cvs.width/2 - 100, cvs.height/2 ); 
    }
}
...
function levelUp(){
    ...
    if(isLevelDone){
        if(LEVEL >= MAX_LEVEL){
            ...
            BG_SOUND.pause();
            WIN_SOUND.play();
            showGamePoints1("You Win", cvs.width/2-45, cvs.height/2);
            return;
        }
        brick.row++;
        createBricks();
